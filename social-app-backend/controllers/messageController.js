...
export async function createMessage(req, res) {
  try {
    ...
    //Tim cuoc tro chuyen co id khop voi id ma minh gui len
    //Neu khong co member $in nay se gui tin nhan khong chinh xac
    const conversation = await Conversation.findOne({ _id: req.body.conversationId, members: { $in: req.user.id } });

    if (!conversation) {
      return errorController.errorHandler(res, "Conversation not found", 404);
    }
    //Neu tin nhan la hinh anh
    //Upload len cloundinary
    //Luu data duoi dang hinh anh
    if (req.body.asset) {
      ...
      await Conversation.findByIdAndUpdate(req.body.conversationId, { lastMessage: message._id });  //Luu tin nhan cuoi cung vao id 
      return;
    }
  } catch (error) {
    return errorController.serverErrorHandler(error, res);

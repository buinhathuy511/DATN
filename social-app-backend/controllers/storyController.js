...
export async function createStoryHandler(req, res) {
  try {
    ...
    if (req.body.asset) {
      const result = await factoryController.uploadFile(req.body.asset.url, "story", req.body.asset.media_type); //day len cloundinary
      req.body.asset = {
        url: result.secure_url, //tra ve URL va luu URL len db
        media_type: result.resource_type,
      };
      req.body.timing = result.duration || 10;  //Gan thoi luong video = duration cua video - neu khong thi mac dinh 10s
    }
    const newStory = await Story.create(req.body);  //Tao Story moi

    const user = await User.findById(req.body.userPost);  //Lay ra id nguoi gui
    
    //Gui thong bao toi ban be
...
//Loai bo nhung story het han
export async function getStoryHandler(req, res) {
  try {
    //Lay nhung story cua userPost (gte : lon hon hoac bang)

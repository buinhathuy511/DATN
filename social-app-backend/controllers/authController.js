...
//hash password nguoi dung 
export function hashingPassword(password) {
  return bcrypt.hashSync(password, bcrypt.genSaltSync(10));
}
//So sanh password nguoi dung nhap vao va password hash
...
export async function forgotHandler(req, res) {
  try {
    ...
    if (existingCode) {
      await ForgotPasswordCode.delete({ email: req.body.email });
    }
